...
As is well-known, the special case of the rule~\eqref{equ:natind} obtained by considering the dependent type in its premiss to be constant
 provides a counterpart of the familiar principle of defintion of a function by recursion:
...
it allows us to define a function $f \co \Nat \to A$ such that each face in the the following diagram commutes:
\[
...
 expect future semantic accounts would be given using homotopy-invariant versions of initials algebras for polynomial
...
the dependent types that determine polynomial functors may have homotopy level greater than~$1$ ({cf.}~\cite{KockJ:dattsp}).
...
\noindent
\emph{Formalization.} All the results in this paper have been formally proved in the current version of 
the Coq proof assistant. The formalization files, which build on the existing libraries for homotopy type theory and 
...
\end{center}
One should note that the type theory underlying the current version of Coq differs from that used in this
paper in several respects. Although the formalization uses only a small fragment of the type theory implemented
in Coq, it does use its judgemental $\eta$-rules for both $\Pi$-types and $\Sigma$-types (see below for details).
The results in this paper are obtained  working instead in a  type theory that assumes 
the  judgemental $\eta$-rules for $\Pi$-types, but not for $\Sigma$-types.
...
Note that none of the type theories we consider in this paper include the judgemental form of the $\eta$-rules for $\Sigma$-types, 
...
\]
is inhabited.  The notion of an equivalence was defined in~\cite{VoevodskyV:unifc} inspired by homotopy-theoretic ideas. It is of
particular importance since it provides a very internal notion of isomorphism between types. 
...
For the convenience of the reader, we list the different kinds of equalities (including logical equivalence) used in the paper in 
...
\end{center}
\end{minipage}} \smallskip
\caption{Symbols for equality relations.}
...
Let $A = (A, a_0, a_1)$ be a bipointed type. Then $A$ is inductive if only if we can derive rules of the form
\begin{enumerate}[(i)]
...
and $B \co A \to \U$, so as to highlight the analogy with the theory of polynomial functors on locally cartesian closed categories~\cite{GambinoN:weltdp,MoerdijkI:weltc}. We shall also use that $P$ acts on homotopies: given
...
\begin{equation}
\label{equ:palpha}
(P \alpha)_{x, u} \defeq (x, \int( \alpha_u ) ) \, ,
\end{equation}
...
we define $e_f \co (\Pi v \co PC) E(\sup_C(v))$ so that, for 
$x \co A$, $u \co B(x) \to C$, we have 
\begin{equation}
\label{equ:ef}
...
PE' \ar[r]_{\sup_{E'}}   & \; E' \, .}
\] 
Let $\phi \defeq (\ext \bar{f})$ be the homotopy associated to the path $\bar{f}$. 
...
\end{equation}
Furthermore, let us define $f_2 \co (\Pi z \co C) E(f_1 z)$ by setting
\[
